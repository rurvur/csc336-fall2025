  ...
  useEffect(() => {
    function loadData() {
      fetch("/api/pokemon")
        .then(res => res.json())
        .then(data => {
          setFromServer({something: data.pokemon.team[1].name});
          setNickname({nname: data.pokemon.team[1].nickname});
          setNature({nature: data.pokemon.team[1].nature});
          setMoves({
            move1: data.pokemon.team[1].moves[0],
            move2: data.pokemon.team[1].moves[1],
            move3: data.pokemon.team[1].moves[2],
            move4: data.pokemon.team[1].moves[3]
          });
          setType1({type1: data.pokemon.team[1].type});
          setType2({type2: data.pokemon.team[1].type2});
          setItem({item: data.pokemon.team[1].item});
          setStats({
            hp: data.pokemon.team[1].EVs.HP,
            attack: data.pokemon.team[1].EVs.Attack,
            defense: data.pokemon.team[1].EVs.Defense,
            spattack: data.pokemon.team[1].EVs.SpAttack,
            spdefense: data.pokemon.team[1].EVs.SpDefense,
            speed: data.pokemon.team[1].EVs.Speed
          });
      });      
    };
